    ...
    <div className="min-h-screen bg-slate-950 bg-grid py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/turkish-companies/new" className="btn-primary flex items-center gap-2">
            <PlusIcon className="h-4 w-4" /> Yeni Sirket
          </Link>
        </div>
        <div className="mb-6">
          <input type="text" placeholder="Sirket adi ile ara..." value={search} onChange={e => setSearch(e.target.value)} className="input-field max-w-md" />
        </div>
        {loading ? (
          ...
          <div className="card p-12 text-center">
            <p className="text-slate-400 mb-4">Henuz turk sirket yok</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-800">
                  ...
                  <th className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase">Mudur</th>
              ...
              <tbody>
                {filtered.map(c => (
                  <tr key={c.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-400">{c.manager_name}</td>
                    ...
                    <td className="px-6 py-4 text-right flex items-center justify-end gap-2">
                      <Link href={`/turkish-companies/${c.id}/edit`} className="text-indigo-400 hover:text-indigo-300 p-1"><PencilIcon className="h-4 w-4" /></Link>
                      <button onClick={() => del(c.id, c.company_name)} className="text-rose-400 hover:text-rose-300 p-1"><TrashIcon className="h-4 w-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
